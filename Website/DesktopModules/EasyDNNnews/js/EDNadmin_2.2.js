eds3_5_jq(document).ready(function ($) {
		...
		google_maps_token = function (token) {
				...
				map_marker_editor = function (edit_marker) {
					var $add_marker_page,
						page_content = '<div class="page add_marker" style="width: ' + token_settings_dimensions.width + 'px; height: ' + token_settings_dimensions.height + 'px;">' +
							...
							'<p class="option"><textarea class="html" name="html" id="edn_maps_token_marker_html"></textarea></p>' +
							'<p class="option"><label for="edn_maps_token_marker_icon">' + localization.gmaps_token_editor_edit_marker_marker_icon + '</label>: <img id="edn_maps_token_marker_icon" class="edNews_mapsTokenMarkerIcon" name="edn_maps_token_marker_icon" src="' + window.edn_module_root + 'images/icons/default_marker.png"></img></p>' +
							//'<p class="option"><img id="edn_maps_token_marker_icon" class="edNews_mapsTokenMarkerIcon" name="edn_maps_token_marker_icon" src="' + window.edn_module_root + 'images/icons/default_marker.png"></img></p>' +
							'<div class="option edNews_modalWrapper" id="eds_edn__mapsiconplaceholder" ></div>' +
					...
					$marker_icon = $add_marker_page.find('img[name="edn_maps_token_marker_icon"]');

					if (edit_marker) {

						$marker_title.val(marker_settings.title);
						$marker_html.val(marker_settings.html);
						if (marker_settings.icon && marker_settings.icon != '' && marker_settings.icon.length > 1)
							$marker_icon.prop('src', marker_settings.icon);
					}
					...
					$marker_center_on_map.click(function () {
						...
						var $trigger = $(this),
							$iconsHtml = "",
							$placeholder = $('#eds_edn__mapsiconplaceholder');

						eds_edn__maps_iconinfos.forEach((x, i) => {
							$iconsHtml += '<li><img class="edn_maps_token_marker_icon_select" data-filename="' + x.filename + '" src="' + window.edn_module_root + 'images/markers/' + x.filename + '"></li>';
						});


						$markerSelectorWindow = $(
							'<div class="edMaps_markerSelectorWindow edNews_modalWindow edNews_smallBottomRight" id="markerSelectorWindow" style="display:none">' +
							'<div class="edMaps_colorSelectorWrapper">' +
							'<ul>' + $iconsHtml +
							'</ul>' +
							'<span class="edNews__markersWindowClose" title="Close">Close</span>' +
							'</div>' +
							'</div>'
						).appendTo($placeholder);

						$('.edn_maps_token_marker_icon_select').click(function () {
							var eds_eds__newIconPath = window.edn_module_root + 'images/markers/' + $(this).data('filename');
							the_marker.setIcon(eds_eds__newIconPath);

							$('#edn_maps_token_marker_icon').prop('src', eds_eds__newIconPath);
							$('#markerSelectorWindow').prop('style', "display:none");
							return false;
						});

						$('.edNews__markersWindowClose').click(function () {
							$('#markerSelectorWindow').prop('style', "display:none");
							return false;
						});

					$marker_icon.click(function () {
						$('#markerSelectorWindow').prop('style', "display:block");
						return false;
					});
					...
					$confirm.click(function () {
						...
						marker_settings.icon = the_marker.icon;
						...
						if (the_marker.icon.indexOf('/images/icons/marker_animirani.gi') !== -1) {
					...
					$cancel.click(function () {
						if (edit_marker) {
							...
							the_marker.setIcon(marker_settings_backup.icon);
						} else
			...
			generate_token = function () {
				...
				for (var i = 0, l = map_markers.length; i < l; i++) {
					...
					token += '(' + meta.latitude + ':' + meta.longitude + '*' + encode_for_token(meta.title) + '*' + encode_for_token(meta.html) + '*' + encode_for_token(meta.icon) + ')';
				}
				...
				(function () {
					...
					if (markers_string != '') {
						...
						for (var i = 0, l = markers.length; i < l; i++) {
							...
							if (value_pairs[3])
								iconFile = decode_from_token(value_pairs[3]);
							marker_settings = {
								latitude: lat_lng[0],
								longitude: lat_lng[1],
				...
				buttons: [
					{
						...
						click: function () {
							...
							eds_edn__gtData = generate_token();

							$('> input[type="hidden"]', $container).val(eds_edn__gtData);
			...
			$map_markers
				...
				.delegate('.marker', 'mouseleave', function () {
					map_markers[$(this).index()].marker.setIcon(map_markers[$(this).index()]._meta.icon);
				});
	...
	$edn_admin.delegate('.gallery_items_table .start_token_generator', 'click', function () {
		...
		!function () {
			var original_token = $clicked.siblings('input.token_box').val(),
				...
				parse_token = function (token) {
					var peaces = token.split('|'),
						params = {
							...
							linktext: peaces[19],
							srcset: peaces[21] == 'False' ? false : true,
						};
				...
				generate_token = function () {
					var token = '[EasyDNNGallery|' + token_params['id'] +
						...
						'|srcset|' + (token_params['srcset'] ? 'True' : 'False');
				...
				$token_onclick_srcset = $('#gallery_item_token_srcset'),
			...
			$token_onclick_srcset[0].checked = token_params.srcset;
			$token_onclick_srcset
				.unbind()
				.change(function () {
					token_params.srcset = this.checked;
